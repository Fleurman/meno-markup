var meno = (function () {
			...
			if (data.isRaw == true) {
				...
				if (match = /^(:{1,6})(\[)? (.+)/.exec(input)) {
					...
					expr = {
						type: "image",
						alt: match[1],
						size: match[3] ? match[3].split(" "):false,
						url: match[4]
					};
				}
				else if (match = /^(-+|\]+|\^+) (.+)/.exec(input)) {
	...
	function produceHTML(tree) {
		...
		for (i in tree) {
			...
			case 'marquee':
				...
				break;
			case 'image':
				elems += "<img " + addInlines() 
				+ "alt='" + t.alt 
				+ "' src='" + t.url + "'"
				+ (t.size?"width='"+t.size[0]+"'"+(t.size[1]?"height='"+t.size[1]+"'":""):"")
				+ ">\n";
				break;
			case 'container':
				elems += "\n<" + t.tag + " " + t.inline + " >\n";
	...
	m.load= function () {
		var menos = document.querySelectorAll("[meno]");
		for (var i = 0; i < menos.length; i++) {
			var file = menos[i].getAttribute("meno");
			m.loadTo(menos[i],file);
			menos[i].className += " meno";
		}
	}
	return m;
})();